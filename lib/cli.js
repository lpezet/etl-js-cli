...
MyClass.prototype.run = function( pSettings, pArgs ) {
	...
	return new Promise( function(resolve, reject) {
		...
		program
			...
			//.alias('update')
			.description('Run activities from file. Specify <etlSet> to run specific ETL set.')
			.action(function(file, etlSet, options){
				that.setThingsUp( options );
				//console.log('#### options:');
				//console.dir( options );
				
				var oTemplate = yamlParse( Fs.readFileSync( file, {encoding: 'utf8'}) );
				var oParameters = {};
				...
				var oMain = new (that.mMainClass)( that.mLogger );
				oMain.run( pSettings, oTemplate, oParameters ).then(function() {
					resolve();
				}, function( pError ) {
					reject( pError );
				});
